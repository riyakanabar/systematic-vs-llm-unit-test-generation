...
def approximate_pc_reverse_shortest_path(pc_fx, epsilon):
    print("this is getting called??")
    optimal_pc_fx = []
    pc_fx_traversal_order = list(reversed(pc_fx[1:-1]))
    print(f"given is {pc_fx}")
    print(f"epsilon is {epsilon}")
    print(f"reversed order is {pc_fx_traversal_order}")
    ...
    while i < n:
        ...
        segment_value = (U_max + L_min) / 2
        optimal_pc_fx.append([pc_fx_traversal_order[i][0], segment_value])
        if not progress:
            i += 1
        else:
            i = k - 1  # Move to next segment
...
def variant(pc_fx, epsilon):
    print(f"given pc_fx is:{pc_fx} and epsilon is {epsilon}")
    optimal_pc_fx = []
    pc_fx_traversal_order = list(pc_fx[1:-1])
    n = len(pc_fx_traversal_order)
    # Compute upper and lower bounds
    U = [pc_fx_traversal_order[i][1] + epsilon for i in range(n)]
    L = [pc_fx_traversal_order[i][1] - epsilon for i in range(n)]
